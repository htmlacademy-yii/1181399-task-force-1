...
<section class="menu-toggle">
    <ul class="menu-toggle__list">
        <li class="menu-toggle__item menu-toggle__item--completed <?= Yii::$app->request->get('status') === 'done' ? 'menu_toggle__item--current' : '' ?>">
            ...
            <a href="<?= Url::toRoute(['list/index', 'status' => 'done']) ?>">
                Завершённые
            </a>
        </li>
        <li class="menu-toggle__item <?= Yii::$app->request->get('status') === 'new' || Yii::$app->request->get('status') === null ? 'menu_toggle__item--current' : '' ?> menu-toggle__item--new">
            <div class="menu-toggle__svg-wrapper">
            ...
            <a href="<?= Url::toRoute(['list/index', 'status' => 'new']) ?>">
                Новые
            </a>
        </li>
        <li class="menu-toggle__item menu-toggle__item--active <?= Yii::$app->request->get('status') === 'wip' ? 'menu_toggle__item--current' : '' ?>">
            <div class="menu-toggle__svg-wrapper">
            ...
            <a href="<?= Url::toRoute(['list/index', 'status' => 'wip']) ?>">
                Активные
            </a>
        </li>
        <li class="menu-toggle__item menu-toggle__item--canceled <?= Yii::$app->request->get('status') === 'canceled' ? 'menu_toggle__item--current' : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <a href="<?= Url::toRoute(['list/index', 'status' => 'canceled']) ?>">
                Отменённые
            </a>
        </li>
        <li class="menu-toggle__item menu-toggle__item--hidden <?= Yii::$app->request->get('status') === 'failed' ? 'menu_toggle__item--current' : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <a href="<?= Url::toRoute(['list/index', 'status' => 'failed']) ?>">
                Проваленные
            </a>
        </li>
    </ul>
    ...
    <div class="my-list__wrapper">
        ...
        <div class="new-task__card">
            <div class="new-task__title">
                <a href="<?= Url::toRoute(['tasks/view', 'id' => $task->id]) ?>" class="link-regular"><h2><?= htmlspecialchars($task->title) ?></h2></a>
                <a  class="new-task__type link-regular" href="#"><p><?= $task->category->name ?></p></a>
            </div>
            <div class="task-status done-status"><?= $task->getStatusName() ?></div>
            ...
            <div class="feedback-card__top ">
                ...
                <div class="feedback-card__top--name my-list__bottom">
                    <p class="link-name"><a href="#" class="link-regular"><?= htmlspecialchars($task->author->name) ?></a></p>
                    <a href="#" class="my-list__bottom-chat  my-list__bottom-chat--new"><b><?= count($task->messages) ?></b></a>
                    <?php if ($task->feedbacks): ?>
                    <span></span><span></span><span></span><span></span><span class="star-disabled"></span>
                    <b><?= $task->feedbacks->rating ?></b>
                    <?php endif; ?>
                </div>
            </div>
        </div>
